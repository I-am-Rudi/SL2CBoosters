...
# Define accessname
const clib = "libsl2cboosters"

# Define path to deps
const depsdir = joinpath(@__DIR__, "..", "deps", "usr")
const libdir = joinpath(depsdir, "lib")
...
function __init__()
    ...
    # First load wigxjpf as it's a dependency
    if !isfile(libwigxjpf)
        error("wigxjpf library not found at $libwigxjpf. Please rebuild the package.")
    end
    
    # Load wigxjpf with RTLD_GLOBAL flag
    wigxjpf_handle = Libdl.dlopen(libwigxjpf, Libdl.RTLD_GLOBAL)
    if wigxjpf_handle == C_NULL
        error("Failed to load wigxjpf: $(Libdl.dlerror())")
    end

    # Then load sl2cboosters
    if !isfile(libsl2c)
        error("sl2cboosters library not found at $libsl2c. Please rebuild the package.")
    end
    
    # Load sl2cboosters with RTLD_GLOBAL flag
    sl2c_handle = Libdl.dlopen(libsl2c, Libdl.RTLD_GLOBAL)
